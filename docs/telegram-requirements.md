...
- **TG-MOVIE-2 — Términos de consulta.** Consultar hasta ~6 títulos candidatos con un máximo de ~14 términos. Variantes por título: base, **cabecera pre-subtítulo** (`Título: Subtítulo` → `Título`) y **cola post-subtítulo** (→ `Subtítulo`), ambas sin partir palabras con guion y exigiendo ≥2 tokens, acrónimo (iniciales, **3–8 letras** en queries y matching; las de 2 letras como `EX` inundan TDLib de ruido y colisionan en matching), sin año, sin marcador de secuela al final (`2`, `II`, `Part 2`), tokens normalizados sin palabras vacías ES/EN más los últimos 2 tokens. Si hay `imdbId` válido (`tt` + 7–9 dígitos), intercalar `variante + imdb` y añadir el `imdb` solo al final. Buscar tanto en mensajes con documento como con vídeo, con límite alto por consulta y corte en el top ~40 global. La cabecera existe porque los archivos suelen llevar solo `Título` y la puntuación por solape nunca alcanza el umbral contra el título oficial largo (ver TG-MOVIE-4); el acrónimo en matching existe porque hay archivos nombrados solo con la sigla (`MIB`).
...
- **TG-MOVIE-4 — Aceptación por título.** Rechazar si los años conocidos difieren en más de 1. Aceptar si la puntuación de título ≥ 0.70, o el fichero menciona el `imdbId`, o la consulta era solo `imdbId`, o **contención de token único**: el nombre limpio es una sola palabra (≥4 letras) contenida en algún título candidato **y** el año es conocido en ambos lados (el gate de año ya garantizó ±1). Esto cubre archivos como `Clave (2008)` para `Expediente X: Creer es la clave`. Bonificar cuando el fichero menciona el `imdbId`. La puntuación combina solape de tokens (con sinónimos ES/EN como hombre/man, etc.) y similitud de texto, penalizando tokens extra.
...
- **TG-SERIES-2 — Formatos reconocidos.** Reconocer `S06E03`/`S6E3`, `6x03`/`12x09` **sin** confundir `720p`/`1080p` con episodio, `Temporada X Capítulo/Episodio Y`, `T02E05`, y `E/Ep/Cap N` suelto. Límites razonables (temporada ≤ 100, episodio ≤ 2000). Extraer además calidad (`480/720/1080/2160p` + `4k`) y año (último `19xx/20xx`). Los dígitos sueltos se conservan en el título limpio (suelen ser número de secuela: `Prada 2`); los marcadores S/E ya se cortaron antes, así que un dígito restante es señal, no ruido.
