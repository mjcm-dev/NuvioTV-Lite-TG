...
object SubtitleCharsetDetector {
    ...
    fun repairDoubleEncodedUtf8IfNeeded(text: String, languageHint: String? = null): String {
        ...
        if (targetCharset != null) {
            ...
            // Hot path: called per ASS dialogue line. A win1255 repair needs >=10 chars that
            // decode into the Hebrew block, and only non-ASCII bytes can, so bail before
            // splitting when there aren't enough.
            if (text.count { it.code >= 0x80 } < 10) return text
            val words = text.split(WHITESPACE_REGEX).filter { it.length >= 2 }
            if (words.isNotEmpty()) {
