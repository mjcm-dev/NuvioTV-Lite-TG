...
object SubtitleCharsetDetector {
    ...
    fun repairDoubleEncodedUtf8IfNeeded(text: String, languageHint: String? = null): String {
        ...
        // Hot path: called per ASS dialogue line. A repair needs >=10 chars that decode into
        // the Hebrew block, and only non-ASCII bytes can, so bail before splitting.
        if (text.count { it.code >= 0x80 } < 10) return text

        val words = text.split(WHITESPACE_REGEX).filter { it.length >= 2 && it.any { c -> c.isLetter() } }
        ...
        if (mojibakeWordsCount >= threshold && (mojibakeWordsCount * 2 >= words.size)) {
            try {
                ...
                if (!CHARSET_WIN1252.newEncoder().canEncode(text)) return text
